...
SELECT departement_nom, ville_departement, COUNT(ville_departement) AS nombre_communes FROM departement,villes_france_free WHERE departement.departement_code = villes_france_free.ville_departement GROUP BY departement_nom ORDER BY COUNT(ville_departement) DESC;
...
SELECT ville_nom, COUNT(*) AS "Nombres de fois qu'il y'a le meme nom de ville" FROM villes_france_free GROUP BY ville_nom HAVING COUNT(*)>1;
